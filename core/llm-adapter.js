...
// ── token 用量计量（日预算 L3 熔断的数据源）──
const usage = { day: dayKey(), tokensIn: 0, tokensOut: 0, calls: 0, errors: 0 };
function dayKey(d = new Date()) { return d.toISOString().slice(0, 10); }
export function getUsage() {
  const today = dayKey();
  if (usage.day !== today) { usage.day = today; usage.tokensIn = 0; usage.tokensOut = 0; usage.calls = 0; usage.errors = 0; }
  return { ...usage };
}
...
function recordUsage(u) {
  usage.tokensIn += u?.prompt_tokens ?? 0;
  usage.tokensOut += u?.completion_tokens ?? 0;
}
...
/** 原始 chat 调用（带重试/超时/熔断）。MOCK 模式走 mockChat。 */
export async function chat({ messages, temperature = 0.2, json = false, maxTokens = 2048 }) {
  if (CONFIG.MOCK) return mockChat({ messages });
...
export async function chatJson({ messages, validate, temperature = 0.2, label = 'json' }) {
  const attempt = async (extraMsg) => {
    const r = await chat({ messages: extraMsg ? [...messages, extraMsg] : messages, temperature, json: true });
 ...
 */
export async function judge({ system, question, options }) {
  ...
  const ask = async () => {
    const r = await chat({
      ...
      temperature: 0, json: false, maxTokens: 16,
    });
...
function mockChat({ messages }) {
  ...
  // 进化钩子：记忆抽取 / 复盘 / 技能提炼（保证 MOCK 模式下进化→净化闭环可演示）
...
function mockJudge(question, options = []) {
  // 冲突/矛盾判定问题 → 一致判 CONFLICT；任务审查 → SUCCESS；其余 → OK（供脏数据注入测试走真管线）
  const conflict = /冲突|矛盾|相反|contradict/i.test(question);
  const taskReview = options.includes('SUCCESS');
  const verdict = conflict ? 'CONFLICT' : taskReview ? 'SUCCESS' : 'OK';
